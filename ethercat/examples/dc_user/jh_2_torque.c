...
void cyclic_task()
{
    ...
    while(1) {
        ...
        for(ii=0;ii<2;ii++){
          ...
          if( (ss&0xFF) ==0x037){  
              tqu = EC_READ_S16(domain1_pd + offset[ii].act_torque);
  
  
              printf("torque: 0x%04x \n",tqu);
              EC_WRITE_S16( domain1_pd + offset[ii].target_torque, 20 );
          }
        }
 ...
 //   uint8_t* data = (uint8_t*)malloc(4*sizeof(uint8_t));
   ...
   // size_t rb;
    ...
    //---------end get master / salve info----------------------
        ...
        // configure SYNC signals for this slave
        ecrt_slave_config_sdo8( sc2, 0x6060, 0, 4 );
        //ecrt_slave_config_sdo16( sc, 0x1C32, 1, 2 );
        //ecrt_slave_config_sdo16( sc, 0x1C33, 1, 2 );
        //ecrt_slave_config_sdo8( sc, 0x60C2, 1, 8 );
        //ecrt_slave_config_sdo8( sc, 0x60C2, 2, -3 );     
