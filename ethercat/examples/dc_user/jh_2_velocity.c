...
const static ec_pdo_entry_reg_t domain1_regs[] = {
    ...
    //{ 0,0, InovanceVidPid, 0x6060, 0, &offset[0].mode_Of_Operation },
    { 0,0, InovanceVidPid, 0x6071, 0, &offset[0].target_torque },
    { 0,0, InovanceVidPid, 0x607a, 0, &offset[0].target_position },
    { 0,0, InovanceVidPid, 0x60b1, 0, &offset[0].offset_velocity },
    { 0,0, InovanceVidPid, 0x60b2, 0, &offset[0].offset_torque },
    { 0,0, InovanceVidPid, 0x60ff, 0, &offset[0].target_velocity },

    //TxPDO-1a02
    { 0,0, InovanceVidPid, 0x6041, 0, &offset[0].status_word },      
    { 0,0, InovanceVidPid, 0x6064, 0, &offset[0].act_position},
    { 0,0, InovanceVidPid, 0x606c, 0, &offset[0].act_velocity},    
    { 0,0, InovanceVidPid, 0x6077, 0, &offset[0].act_torque },
    { 0,0, InovanceVidPid, 0x6061, 0, &offset[0].mode_Of_Operation_dsiplay },      
    
    //{ 0,0, InovanceVidPid, 0x6061, 0, &offset[0].mode_Of_Operation_dsiplay },      
       

    { 0,1, InovanceVidPid, 0x6040, 0, &offset[1].ctrl_word },
    //{ 0,0, InovanceVidPid, 0x6060, 0, &offset[1].mode_Of_Operation },
    { 0,1, InovanceVidPid, 0x6071, 0, &offset[1].target_torque },
    { 0,1, InovanceVidPid, 0x607a, 0, &offset[1].target_position },
    { 0,1, InovanceVidPid, 0x60b1, 0, &offset[1].offset_velocity },
    { 0,1, InovanceVidPid, 0x60b2, 0, &offset[1].offset_torque },
    { 0,1, InovanceVidPid, 0x60ff, 0, &offset[1].target_velocity },

    //TxPDO-1a02
    { 0,1, InovanceVidPid, 0x6041, 0, &offset[1].status_word },      
    { 0,1, InovanceVidPid, 0x6064, 0, &offset[1].act_position},
    { 0,1, InovanceVidPid, 0x606c, 0, &offset[1].act_velocity},    
    { 0,1, InovanceVidPid, 0x6077, 0, &offset[1].act_torque },
    { 0,1, InovanceVidPid, 0x6061, 0, &offset[1].mode_Of_Operation_dsiplay },   
    {}
};
    ...
    while(1) {
        ...
        for(ii=0;ii<2;ii++){
          ...
          if( (ss&0xFF) ==0x37){  
              ...
              printf("velocity %d : 0x%04x \n",ii,vel);
              EC_WRITE_S32( domain1_pd + offset[ii].target_velocity, 13107*6 );
 ...
 //   uint8_t* data = (uint8_t*)malloc(4*sizeof(uint8_t));
   ...
   // size_t rb;
    ...
    //---------end get master / salve info----------------------
        ...
        ecrt_slave_config_sdo8( sc, 0x6060, 0, 3 );
        ...
        if (!(sc2 = ecrt_master_slave_config( master,0,1 ,InovanceVidPid)) ) {
            fprintf(stderr, "Failed to get slave configuration for IS620N.\n");
            exit( EXIT_FAILURE );
        }

        printf("Configuring PDOs...\n");
        if ( ecrt_slave_config_pdos( sc2, EC_END, IS620N_syncs ) ) {
            fprintf( stderr, "Failed to configure IS620N PDOs.\n" );
            exit( EXIT_FAILURE );
        }
        ...
        // configure SYNC signals for this slave
        ecrt_slave_config_sdo8( sc2, 0x6060, 0, 3 );
        //ecrt_slave_config_sdo16( sc, 0x1C32, 1, 2 );
        //ecrt_slave_config_sdo16( sc, 0x1C33, 1, 2 );
        //ecrt_slave_config_sdo8( sc, 0x60C2, 1, 8 );
        //ecrt_slave_config_sdo8( sc, 0x60C2, 2, -3 );     
